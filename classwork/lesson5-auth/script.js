...
; (function () {
  ...
  app.config(($stateProvider, $urlRouterProvider, toastrConfig) => {
    ...
    $stateProvider.state('common', {
      resolve: {
        auth: (AuthService, $state) => AuthService.isAuth().catch(() => {
          $state.go('login');
        })
      }
    })
    ...
    $stateProvider
      ...
      .state('user.list', {
        ...
        component: 'userList',
        resolve: {
          cards: (users, CardsService) => userService.getUsers()
        }
      })
      .state('user.card', {
        name: 'userCard',
  ...
  app.run(($scope, $rootScope, $state, authService, $transitions, toastr) => {

    //$scope.brodcast('something')
    //$scope.emit('something')
    //$scope.on('something', () => {});

    $rootScope.$on('$stateChangeStart', function (event, toState) {
      if (toState !== 'login' && !AuthService.isAuth()) {
  ...
  //<input ng-model="field" my-validation>
...
- resolve
- stateChange (events / hooks(1.X+))
- API: 401 (httpInterceptor)
